...
int main(int argc, char* argv[])
{
    ...
    if (localSN_left.empty() || remoteSN_left.empty() || licCfgPath.empty()) {
        printHelp();
        return 1;
    }
    ...
    std::cout << "Remote SN: " + remoteSN_left << std::endl;
    std::cout << "Local SN: " + localSN_left << std::endl;
    std::cout << "License config file: " + licCfgPath << std::endl;

    try {

        flexiv::omni::teleop::Robot2RobotTeleop teleop(localSN_left, remoteSN_left, licCfgPath);
